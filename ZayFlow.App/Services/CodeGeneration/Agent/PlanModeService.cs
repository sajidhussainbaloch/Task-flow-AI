...
/// <summary>
/// Phase 3 â€” Plan Mode: AI proposes a structured execution plan before performing actions.
...
public sealed class PlanModeService
{
    ...
    public PlanModeService(CloudflareProvider openRouterProvider, ILogger<PlanModeService> logger)
    {
...
""";
    ...
    public static string FormatPlan(ExecutionPlan plan)
    {
        ...
        sb.AppendLine($"### ðŸ“‹ Execution Plan: {plan.Title}");
        ...
        foreach (var step in plan.Steps)
        {
            ...
            var reversibleTag = step.Reversible ? "" : " âš ï¸irreversible";
            ...
            sb.AppendLine($"   Action: `{step.Action}` â†’ `{step.Target}`");
    ...
    {
        var completed = plan.Steps.Count(s => s.Status == PlanStepStatus.Completed);
        return $"âœ… Step {step.Id}/{plan.Steps.Count}: {step.Description}\n" +
               $"   {step.ResultSummary}\n" +
