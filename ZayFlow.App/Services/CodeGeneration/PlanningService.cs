...
/// <summary>
/// Planning Layer â€” converts a <see cref="NormalizedBrief"/> into a structured
...
public sealed class PlanningService
{
    ...
    public PlanningService(CloudflareProvider openRouterProvider, ILogger<PlanningService> logger)
    {
...
### USE single-file (DEFAULT â€” use this unless multi-file is REQUIRED)
single-file means: 1 step, 1 file, ALL code inside that file.

These are ALL single-file â€” even with GUI:
- Python anything: tkinter GUI, pygame, flask server, CLI tool, script, bot â€” Python can do everything in 1 file
- HTML page (inline CSS + JS in one .html file)
- JavaScript/Node.js: server, tool, bot â€” 1 file
- C/C++ program â€” 1 file
- Java program â€” 1 file (one class with main)
- Go program â€” 1 file
- Rust program â€” 1 file
...
"Can this ENTIRE program run from a single file in this language?" If YES â†’ single-file.
...
""";
    ...
    private static string BuildUserPrompt(NormalizedBrief brief)
    {
        ...
        // â”€â”€ Phase 3: inject blueprint context for precise planning â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        if (brief.Blueprint != null && brief.Blueprint.Complexity != ProjectComplexity.SingleFile)
        {
            sb.AppendLine();
            sb.AppendLine("## PROJECT BLUEPRINT (from Project Intelligence â€” follow this exactly)");
            ...
            {
                sb.AppendLine($"- **{file.RelativePath}** â€” {file.Description}");
