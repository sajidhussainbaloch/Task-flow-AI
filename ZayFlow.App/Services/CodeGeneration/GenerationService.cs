...
/// <summary>
/// Generation Layer â€” produces complete code implementations from the plan.
/// Outputs <see cref="GeneratedFile"/> artifacts with full, runnable code â€” no placeholders.
/// </summary>
public sealed class GenerationService
{
    ...
    public GenerationService(CloudflareProvider openRouterProvider, ILogger<GenerationService> logger)
    {
        ...
        CancellationToken ct = default)
    {
        ...
        // Process steps in waves: each wave contains steps whose dependencies are already completed
        while (completedSteps.Count < orderedSteps.Count)
        {
            ct.ThrowIfCancellationRequested();

            var readySteps = orderedSteps
                .Where(s => !completedSteps.Contains(s.Order))
                .Where(s => s.DependsOn.Count == 0 || s.DependsOn.All(dep =>
                    completedSteps.Any(completed => orderedSteps.Any(os => os.Order == completed &&
                        os.TargetFile.Equals(dep, StringComparison.OrdinalIgnoreCase)))))
                .ToList();

            // Fallback: if no steps are ready (circular/unresolvable deps), take the first unfinished
            if (readySteps.Count == 0)
            {
                readySteps = orderedSteps.Where(s => !completedSteps.Contains(s.Order)).Take(1).ToList();
            }

            if (readySteps.Count == 1)
            {
                // Single step — run sequentially with dependency context
                var step = readySteps[0];
                var file = await GenerateStepAsync(plan, step, files, model, ct).ConfigureAwait(false);
                files.Add(file);
            ...
            else
            {
                ...
                var snapshot = files.ToList(); // freeze context for parallel tasks
                var tasks = readySteps.Select(step => GenerateStepAsync(plan, step, snapshot, model, ct)).ToList();
                var results = await Task.WhenAll(tasks).ConfigureAwait(false);
                foreach (var file in results)
                {
                    files.Add(file);
                    var matchingStep = readySteps.FirstOrDefault(s =>
                        s.TargetFile.Equals(file.RelativePath, StringComparison.OrdinalIgnoreCase));
                    if (matchingStep != null)
                        completedSteps.Add(matchingStep.Order);
                }
            }
        }
    ...
    private async Task<GeneratedFile> GenerateStepAsync(
        ImplementationPlan plan,
        PlanStep step,
        List<GeneratedFile> previousFiles,
        string model,
        CancellationToken ct)
    {
        var systemPrompt = BuildSystemPrompt(plan.Brief.Language);
        var userPrompt = BuildUserPrompt(plan, step, previousFiles);
...
9. Programs MUST be INTERACTIVE by default. Use input() prompts, menus, and loops â€” NOT sys.argv or command-line arguments.
...
{""intent"":""create_file"",""message"":""<brief summary of what was generated>"",""parameters"":{""fileName"":""<filename.ext>"",""language"":""" + language + @""",""content"":""<complete source code with \n for newlines>"",""rationale"":""<brief explanation of implementation choices>""}}";
    ...
    private static string BuildUserPrompt(ImplementationPlan plan, PlanStep step, List<GeneratedFile> previousFiles)
    {
        ...
        // â”€â”€ Phase 3: inject blueprint content contract for this specific file â”€â”€â”€â”€â”€
        ...
        // â”€â”€ Phase 2: repo-aware edit context â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
