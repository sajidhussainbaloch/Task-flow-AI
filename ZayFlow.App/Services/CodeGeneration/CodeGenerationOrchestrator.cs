...
/// Runs the full pipeline: Understanding → Planning → Generation → Verification → Improvement → Output.
...
public sealed class CodeGenerationOrchestrator
{
        ...
        CancellationToken ct = default)
    {
        ...
        // ── Phase 2: multi-turn continuation ───────────────────────────────────────
        // If the caller attached a previous session, continue from its last state
        // (skip Understanding + Planning, jump straight to Improvement → Verification loop).
        ...
        try
        {
            ...
            session.Brief = await RunStageAsync(
                ...
                brief => $"Understood {brief.Language} request with risk {brief.RiskLevel}.").ConfigureAwait(false);
            session.Brief.RepositoryContext = session.RepositoryContext;

            // ── Phase 3: project intelligence — produce blueprint before planning ───
            EmitProgress(session, request.CodeProgress, CodeGenerationStage.Understanding, CodeGenerationEventStatus.Started,
            ...
            _logger.LogInformation("Code engine: blueprint produced — IsKnown={IsKnown}, Complexity={Complexity}, Files={FileCount}",
            ...
            session.Plan = await RunStageAsync(
                ...
                request.CodeProgress,
                plan => $"Planned {plan.Steps.Count} step(s) as {plan.ChangeType}.").ConfigureAwait(false);
            ...
            // ── Phase 2: populate EditStrategy on plan steps ─────────────────────────
            ...
            EmitProgress(session, request.CodeProgress, CodeGenerationStage.Generation, CodeGenerationEventStatus.Started,
                $"Generating {session.Plan.Steps.Count} file(s)...", 40, generatedFileCount: session.Plan.Steps.Count);
    ...
    /// <summary>
    /// Phase 2 — Multi-turn continuation: the user is refining or extending a previous session.
        ...
        CancellationToken ct)
    {
        ...
        // ── Phase 2: persist session to memory ────────────────────────────────────
        try
        {
        ...
        // ── Phase 3: audit trail flush on completion ──────────────────────────────
    ...
    /// <summary>
    /// Phase 2 — Populate EditStrategy and ExistingContent on plan steps.
