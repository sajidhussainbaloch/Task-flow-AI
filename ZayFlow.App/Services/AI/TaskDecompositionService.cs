...
/// <summary>
/// Phase 2 — Decomposes complex user requests into sub-tasks with dependencies.
...
public sealed class TaskDecompositionService
{
    ...
    public TaskDecompositionService(OpenRouterProvider openRouterProvider, ILogger<TaskDecompositionService> logger)
    {
...
- Keep tasks granular but not trivially small — each should produce a visible outcome.
- Category determines how the task will be routed (code → code engine, docs → document pipeline, etc.).
...
""";
    ...
    public static string FormatPlan(TaskPlan plan)
    {
        ...
        sb.AppendLine($"### 📋 Task Plan: {plan.Summary}");
        sb.AppendLine();
