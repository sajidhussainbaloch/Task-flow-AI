...
/// <summary>
/// Main AI service that orchestrates Groq-backed assistant turns, token management, and execution.
/// </summary>
public class AIService : IAIService
{
        ...
        ILogger<AIService> logger)
    {
        ...
        {
            openRouterProvider.Initialize(preferences.OpenRouterApiKey);
    ...
    public async Task<ChatMessage> ProcessAssistantTurnAsync(AssistantTurnRequest request, CancellationToken ct = default)
    {
        ...
        if (_currentProvider == null)
        {
            return new ChatMessage { Content = "No AI provider configured. Please configure Groq in settings.", Role = "assistant" };
        }
    ...
    {
        if (_currentProvider is not OpenRouterProvider orProvider)
        {
            return new ImageGenerationResult { Success = false, Message = "Image generation requires OpenRouter provider." };
        }
    ...
    public void ClearHistory()
    {
        ...
        if (_currentProvider is OpenRouterProvider orProvider)
        {
