...
{
    /// <summary>Raised when the view should scroll to the bottom (e.g. during streaming).</summary>
    ...
    private string _typingStatusText = "Thinking...";
    private double _typingProgress = -1;
    private string _typingStageIcon = "⏳";
    ...
    public string TypingStatusText
    {
        get => _typingStatusText;
        set => SetProperty(ref _typingStatusText, value);
    }

    public double TypingProgress
    {
        get => _typingProgress;
        set => SetProperty(ref _typingProgress, value);
    }

    public string TypingStageIcon
    {
        get => _typingStageIcon;
        set => SetProperty(ref _typingStageIcon, value);
    }
    ...
    private async void OnSend(object? _)
    {
        ...
        if (IsPlanMode && _orchestrator.GetPendingPlan() == null && !ShowingPlan)
        {
            try
            {
                using var planCts = new CancellationTokenSource(TimeSpan.FromSeconds(300));
                _activeExecutionCts = planCts;
                RaiseCommandStates();
        ...
        try
        {
            ...
            if (_preferencesService.Get().EnableStreamingResponses)
            {
                ...
                request.CodeProgress = new Progress<CodeGenerationStageEvent>(evt =>
                {
                    ...
                    // Update the typing indicator with live stage info
                    TypingStageIcon = GetStageIcon(evt.Stage);
                    TypingStatusText = evt.Stage switch
                    {
                        CodeGenerationStage.Understanding => "Analyzing your request...",
                        CodeGenerationStage.Planning => "Planning implementation...",
                        CodeGenerationStage.Generation => $"Writing code... ({evt.GeneratedFileCount} file{(evt.GeneratedFileCount != 1 ? "s" : "")})",
                        CodeGenerationStage.Verification => "Verifying & validating...",
                        CodeGenerationStage.Improvement => "Improving code quality...",
                        CodeGenerationStage.Finalization => "Finalizing output...",
                        _ => "Processing..."
                    };
                    TypingProgress = evt.Percent >= 0 ? evt.Percent : -1;
                });
            }
            ...
            var fullContent = assistantMessage.Content;

            // Start with empty content for streaming animation
            assistantMessage.Content = string.Empty;
            Messages.Add(assistantMessage);
            UpdateArtifactPane(assistantMessage);
            UpdateLastCreatedFilePath(assistantMessage);

            // Animate the text streaming in (fire-and-forget on UI thread)
            _ = AnimateContentAsync(assistantMessage, fullContent);
    ...
    /// <summary>
    /// Reveals message content progressively with a natural typing cadence.
    /// Uses word-level chunks with variable speed to feel organic.
    /// </summary>
    private async Task AnimateContentAsync(UIChatMessage message, string fullContent)
    {
        ...
        // For short messages or code-heavy content, show immediately
        ...
        // Large code blocks (1000+ chars) — fast reveal by lines instead of words
        if (fullContent.Length > 1000)
        {
            var lines = fullContent.Split('\n');
            var sb = new System.Text.StringBuilder(fullContent.Length);
            int lineChunk = 3;
            int scrollCounter = 0;

            for (int i = 0; i < lines.Length; i += lineChunk)
            {
                var end = Math.Min(i + lineChunk, lines.Length);
                for (int j = i; j < end; j++)
                {
                ...
                // Accelerate rapidly
                if (i > 15) lineChunk = 6;
                if (i > 40) lineChunk = 12;
                if (i > 80) lineChunk = 25;

                await Task.Delay(lineChunk <= 6 ? 12 : 5);
            }
        ...
        // Normal text — word-by-word with acceleration
        var words = fullContent.Split(' ');
        var wordSb = new System.Text.StringBuilder(fullContent.Length);
        int chunkSize = 3;
        int scrollCtr = 0;
        ...
        for (int i = 0; i < words.Length; i += chunkSize)
        {
            ...
            for (int j = i; j < end; j++)
            {
                if (wordSb.Length > 0) wordSb.Append(' ');
                wordSb.Append(words[j]);
            }

            message.Content = wordSb.ToString();
            if (++scrollCtr % 4 == 0)
                ScrollRequested?.Invoke();

            if (i > 15) chunkSize = 6;
            if (i > 40) chunkSize = 12;
            if (i > 80) chunkSize = 20;

            await Task.Delay(chunkSize <= 6 ? 20 : 8);
        }
